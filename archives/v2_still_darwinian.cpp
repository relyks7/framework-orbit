...
class trait{
    public:
        ...
        float noise;
        float plasticity;
        trait(float initval, float initdecay, float initcap, float initshare, float initnoise, float initplasticity){
            val=initval;
            target_val=initval;
            decay=initdecay;
            voltage=0.0f;
        ...
        void step(float feeder_val, float share_val, float volt_spike){
            target_val=target_val+dt*(plasticity*(feeder_val+share*share_val)-decay*target_val+gaussian_noise(0.0f, noise));
            voltage+=volt_spike;
            if (voltage>capacity){
                voltage=0;
                val=target_val;
