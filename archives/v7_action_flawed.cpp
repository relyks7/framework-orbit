...
class lupus_w{
    public:
        ...
        float slow_learn, fast_learn, eps, tanh_mag, noise_mag, min_noise, max_noise, t_noise;
        float lambda_min, lambda_s, t_lambda;
        bool has_prev=false;
        ...
        void reset(){
            ...
            add_mirror_edge(1,4,1,7);
            add_mirror_edge(4,5,7,8);
            add_mirror_edge(5,6,8,9);
            add_mirror_edge(6,0,9,2);
            add_mirror_edge(1,5,1,8);
            add_mirror_edge(5,0,8,2);
            add_mirror_edge(4,6,7,9);
        ...
        lupus_w(float un, float ud, float sl, float fl, float e, float tm, float nm, float min_n, float max_n, float t_n, float l_m, float l_s, float t_l, lupus_k uk){
            n=un; d=ud;
            slow_learn=sl; fast_learn=fl; eps=e; tanh_mag=tm; k=uk; noise_mag=nm; min_noise=min_n; max_noise=max_n;
            lambda_min=l_m; lambda_s=l_s; t_lambda=t_l;
            t_noise=t_n;
            reset();
        }
        void forward(){
            ...
            for (auto [xx,yy]:mirror){
                auto [x1, x2]=xx;
                auto [y1, y2]=yy;
                float lambda=lambda_min+(lambda_s-lambda_min)*expf(-tick/t_lambda);
                float c=expf(-2*lambda*dt);
                for (int i=0;i<d*d;i++){
                    float w1=(adj[x1][x2].w[i]+adj[y1][y2].w[i])/2;
                    float w2=(adj[x1][x2].w[i]-adj[y1][y2].w[i])/2;
                    adj[x1][x2].w[i]=w1+c*w2;
                    adj[y1][y2].w[i]=w1-c*w2;
                }
            }
        }
        vector<float> generate(vector<float> ipt, vector<float> prior, int settle_steps){
            if (has_prev){
                ...
                for (int i=0;i<d;i++) h[3*d+i]=prev_act[i];
            ...
            for (int i=0;i<d;i++) {
                act_k[i]+=gaussian_noise(0.0f,min(max_noise, max(min_noise*expf(-tick/t_noise), noise_mag*act_rms)));
                ret[i]=dt*act_k[i];
            }
            prev_act=act_k;
            prev_env=ipt;
...
int main(){
    if (keketrial){
        ...
        for (int _=0;_<total;_++){
            lupus_k keke(3, 4, 0.0f, 100000.0f, 0.0f);
            lupus_w sextus(10, 4, 0.01f, 5.0f, 1.0f, 8.0f, 0.1f, 0.05f, 0.2f, 100000.0f, 0.01f, 1.0f, 5000.0f, keke);
            ...
            for (int i=0;i<trial.size();i++){
                ...
                for (int j=0;j<100000;j++){
                    ...
                    float theta=0.75f*pival*sinf(2.0f*pival*sextus.tick/period);
                    ...
                    pend.push_back(mv_j);
                    if (pend.size()>delay){
                        vector<float> lst=pend.front(); pend.pop_front();
                        cw+=lst[0]; cx+=lst[1]; cy+=lst[2]; cz+=lst[3];
                    }
