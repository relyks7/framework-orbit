...
///////note: ignore all of these definitions, everything is being revamped!!! (they are roughly accurate in what they are doing but things are changing)
...
struct node{
    ...
    vector<float> dh_dt; //first derivative of h
    vector<vector<float>> da_dt; //first derivative of a
    vector<float> err; //error
    vector<float> out_down; //emission down to children (belief)
    vector<float> out_up; //emission up to parent (error)
...
void update(){
    ...
    for (int i=0;i<n;i++){
        ...
        if (!nodes[i].is_prior){
            for (auto x:adj[i]){
                ...
                for (int j=0;j<r;j++){
                    z_in[j]+=x.w*nodes[x.from].out_down[j];
                }
                iptn++;
            }
            for (auto x:adj[i]){
        ...
        ///////note: voltage is cut because spiking isn't actually too useful here, the energy equation is the previous version
        ...
        float da_mag=0.0f;
        for (int j=0;j<d;j++){
            dh_mag+=nodes[i].dh_dt[j]*nodes[i].dh_dt[j];
            for (int k=0;k<r;k++){
                da_mag+=nodes[i].da_dt[j][k]*nodes[i].da_dt[j][k];
            }
        }
        nodes[i].e+=dt*((1.0/(nodes[i].u+1.0f))*(nodes[i].xi*nodes[i].delta*sig_mag-(1.0f-nodes[i].xi)*nodes[i].mu*surprise) - chi*dh_mag - zeta*nodes[i].e - nodes[i].omicron*da_mag);
        ...
        if (nodes[i].is_input || nodes[i].is_prior){
            ...
            if (!nodes[i].is_prior) delta_rule(nodes[i].a, nodes[i].err, new_h[i], dt*nodes[i].eta*abs(tanhf(nodes[i].stress)), nodes[i].da_dt);
        } else {
            // if (nodes[i].v>nodes[i].theta){
            nodes[i].stress+=surprise*dt;
            nodes[i].out_down=matvec(nodes[i].b_t,nodes[i].h);
            nodes[i].out_up=matvec(nodes[i].b_t,nodes[i].err);
            //nodes[i].v=0.0f;
            delta_rule(nodes[i].a, nodes[i].err, new_h[i], dt*nodes[i].eta*abs(tanhf(nodes[i].stress)), nodes[i].da_dt);
            // } else{
    ...
    for (int i=0;i<n;i++){
        ...
        for (int j=0;j<d;j++) nodes[i].dh_dt[j]=(new_h[i][j]-nodes[i].h[j])/dt;
        nodes[i].h=new_h[i];
    }
    int tn=n;
    for (int i=0;i<tn;i++){
...
/////// main is commented out for now due to errors
// int main(){
//     //index0: input 1
